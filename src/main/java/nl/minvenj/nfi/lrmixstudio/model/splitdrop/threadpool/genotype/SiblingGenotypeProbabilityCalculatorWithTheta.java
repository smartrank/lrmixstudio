 ...
 * @author dejong
 */
class SiblingGenotypeProbabilityCalculatorWithTheta extends AbstractGenotypeProbabilityCalculator {

    private static final Logger LOG = new ValidationLogger(LoggerFactory.getLogger(SiblingGenotypeProbabilityCalculatorWithTheta.class));
    ...
    public SiblingGenotypeProbabilityCalculatorWithTheta(Hypothesis hypothesis) {
    ...
    private double homozygoteReferenceHeterozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        if (subjectLocus.getAlleles().contains(referenceAllele)) {
            LOG.info("Sibling Genotype Probability for reference {}, subject {} (aa-ab)", toLogString(referenceLocus), toLogString(subjectLocus));
            ...
            LOG.info("  ( ( ( 1 - theta ) * p{} ) / ( 2 * ( 1 + theta ) ) ) * ( 1 + ( ( 2 * theta + ( 1 - theta ) * p{} ) / ( 1 + 2 * theta ) ) )", nonMatching.getAllele(), referenceAllele.getAllele());
            return ((_oneMinusTheta * _populationStatistics.getProbability(subjectLocus, nonMatching)) / _twoTimesOnePlusTheta) * (1 + ((_twoTheta + _oneMinusTheta * _populationStatistics.getProbability(subjectLocus, referenceAllele)) / (_onePlusTwoTheta)));
        }
        // Parent and child do not share any alleles
        LOG.info("Sibling Genotype Probability for reference {}, subject {} (aa-bc)", toLogString(referenceLocus), toLogString(subjectLocus));
        LOG.info("  ( ( 1 - theta )^2 * p{} * p{} ) / ( 2 * ( 1 + theta ) * ( 1 + 2 * theta ) )", subjectAlleles[0].getAllele(), subjectAlleles[1].getAllele());
        return ((1 - _theta) * (1 - _theta) * _populationStatistics.getProbability(subjectLocus, subjectAlleles[0]) * _populationStatistics.getProbability(subjectLocus, subjectAlleles[1])) / (2 * (1 + _theta) * (1 + 2 * _theta));
    }
    ...
    private double homozygoteReferenceHomozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        if (referenceLocus.getAlleles().contains(subjectAllele)) {
            LOG.info("Sibling Genotype Probability for reference {}, subject {} (aa-aa)", toLogString(referenceLocus), toLogString(subjectLocus));
            LOG.info("  ( 1 + ( ( 2 * ( 2 * theta + ( 1 - theta ) * p{} ) ) / ( 1 + theta ) ) + ( ( ( 2 * theta + ( 1 - theta ) * p{} ) * (3 * theta + ( 1 - theta ) * p{} ) ) / ( ( 1+theta ) * ( 1 + 2 * theta ) ) ) ) / 4", subjectAllele.getAllele(), subjectAllele.getAllele(), subjectAllele.getAllele());
            double p = _populationStatistics.getProbability(referenceLocus, subjectAllele);
            return (1 + ((2 * (_twoTheta + (_oneMinusTheta) * p)) / (_onePlusTheta)) + (((_twoTheta + (_oneMinusTheta) * p) * (3 * _theta + (_oneMinusTheta) * p)) / ((_onePlusTheta) * (_onePlusTwoTheta)))) / 4;
        }
        // Parent and child alleles do not match
        LOG.info("Sibling Genotype Probability for reference {}, subject {} (aa-bb)", toLogString(referenceLocus), toLogString(subjectLocus));
        LOG.info("  ( ( ( ( 1 - theta ) * p{} ) * ( theta + ( 1 - theta ) * p{} ) ) ) / ( 4 * ( 1 + theta ) * ( 1 + 2 * theta ) )", subjectAllele.getAllele(), subjectAllele.getAllele());
        double p = _populationStatistics.getProbability(referenceLocus, subjectAllele);
        return ((_oneMinusTheta) * p * (_theta + (_oneMinusTheta) * p)) / (4 * (_onePlusTheta) * (_onePlusTwoTheta));
    }
    ...
    private double heterozygoteReferenceHomozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        if (referenceLocus.hasAllele(subjectAllele.getAllele())) {
            ...
            LOG.info("Sibling Genotype Probability for reference {}, subject {} (ab-aa)", toLogString(referenceLocus), toLogString(subjectLocus));
            LOG.info("  ( ( theta + ( 1 - theta ) * p{}) / ( 4 * ( 1 + theta ) ) ) * (  1 + ( (2 * theta + ( 1 - theta ) * p{} ) / ( 1 + 2 * theta ) ) )", subjectAllele, subjectAllele);
            return ((_theta + _oneMinusTheta * pSubject) / (4 * _onePlusTheta)) * (1 + ((_twoTheta + _oneMinusTheta * pSubject) / _onePlusTwoTheta));
        }

        LOG.info("Sibling Genotype Probability for reference {}, subject {} (ab-cc)", toLogString(referenceLocus), toLogString(subjectLocus));
        LOG.info("  ( ( theta + ( 1 - theta ) * p{} ) * ( 1 - theta ) * p{} ) / ( 4 * ( 1 + theta ) * ( 1 + 2 * theta ) )", subjectAllele.getAllele(), subjectAllele.getAllele());
        return ((_theta + (1 - _theta) * pSubject) * (1 - _theta) * pSubject) / (4 * (1 + _theta) * (1 + 2 * _theta));
    }
    ...
    private double heterozygoteReferenceHeterozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        switch (match.size()) {
            case 0: // No matching alleles (ab-cd)
                LOG.info("Sibling Genotype Probability for reference {}, subject {} (ab-cd)", toLogString(referenceLocus), toLogString(subjectLocus));
                Allele[] mismatchArray = mismatch.toArray(new Allele[mismatch.size()]);
                LOG.info("  ( ( 1 - theta ) * ( 1 - theta ) * p{} * p{} ) / ( 2 * ( 1 + theta ) * ( 1 + 2 * theta ) )", mismatchArray[0].getAllele(), mismatchArray[1].getAllele());
                double p = 1;
                for (Allele a : mismatch) {
                    p *= (1 - _theta) * _populationStatistics.getProbability(subjectLocus, a);
                }
                return p / (2 * (1 + _theta) * (1 + 2 * _theta));
            case 1: // One matching allele (ab-ac)
                LOG.info("Sibling Genotype Probability for reference {}, subject {} (ab-ac)", toLogString(referenceLocus), toLogString(subjectLocus));
                ...
                LOG.info("  ( ( ( 1 - theta ) * p{} ) / ( 4 * (1 + theta))) * ( 1 + ( ( 2 * ( theta + (1 - theta) * p{} ) ) / ( 1 + 2 * theta ) ) ) ", mismatchAllele.getAllele(), matchAllele.getAllele());
                return ((_oneMinusTheta * pMismatch) / _fourTimesOnePlusTheta) * (1 + ((2 * (_theta + (1 - _theta) * pMatch)) / (1 + 2 * _theta)));
            case 2: // All alleles match (ab-ab)
                LOG.info("Sibling Genotype Probability for reference {}, subject {} (ab-ab)", toLogString(referenceLocus), toLogString(subjectLocus));
                Allele[] alleles = match.toArray(new Allele[2]);
                LOG.info("  ( 1 + ( (2 * theta + (1 - theta) * ( p{} + p{} ) ) / ( 1 + theta ) ) + ( ( 2 * ( theta + ( 1 - theta ) * p{} ) * ( theta + ( 1 - theta ) * p{} ) ) / ( ( 1 + theta ) * ( 1 + 2 * theta ) ) ) ) / 4", alleles[0], alleles[1], alleles[0], alleles[1]);
                ...
                return (1 + ((2 * _theta + (1 - _theta) * (pA + pB)) / (1 + _theta)) + ((2 * (_theta + (1 - _theta) * pA) * (_theta + (1 - _theta) * pB)) / ((1 + _theta) * (1 + 2 * _theta)))) / 4;
            default:
