 ...
 * @author dejong
 */
class CousinsGenotypeProbabilityCalculatorWithTheta extends AbstractGenotypeProbabilityCalculator {

    private static final Logger LOG = new ValidationLogger(LoggerFactory.getLogger(CousinsGenotypeProbabilityCalculatorWithTheta.class));
    ...
    public CousinsGenotypeProbabilityCalculatorWithTheta(Hypothesis hypothesis) {
    ...
    private double homozygoteReferenceHeterozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        if (subjectLocus.getAlleles().contains(referenceAllele)) {
            LOG.info("Cousins Genotype Probability for reference {}, subject {} (aa-ab)", toLogString(referenceLocus), toLogString(subjectLocus));
            ...
            LOG.info("  ( ( ( 1 - theta ) * p{} ) / ( 4 * ( 1 + theta ) ) ) * ( 1 + ( ( 6 * ( 2 * theta + ( 1 - theta ) * p{} ) ) / ( 1 + 2 * theta ) ) )", nonMatching.getAllele(), referenceAllele.getAllele());
            ...
            return (((1 - _theta) * pB) / (4 * (1 + _theta))) * (1 + ((6 * (2 * _theta + (1 - _theta) * pA)) / (1 + 2 * _theta)));
        }
        // Reference and subject do not share any alleles
        LOG.info("Cousins Genotype Probability for reference {}, subject {} (aa-bc)", toLogString(referenceLocus), toLogString(subjectLocus));
        LOG.info("  ( 3 * ( 1 - theta )^2 * p{} * p{} ) / ( 2 * ( 1 + theta ) * ( 1 + 2 * theta ) )", subjectAlleles[0].getAllele(), subjectAlleles[1].getAllele());
        return (3 * (1 - _theta) * (1 - _theta) * _populationStatistics.getProbability(subjectLocus, subjectAlleles[0]) * _populationStatistics.getProbability(subjectLocus, subjectAlleles[1])) / (2 * (1 + _theta) * (1 + 2 * _theta));
    }
    ...
    private double homozygoteReferenceHomozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        if (referenceLocus.getAlleles().contains(subjectAllele)) {
            LOG.info("Cousins Genotype Probability for reference {}, subject {} (aa-aa)", toLogString(referenceLocus), toLogString(subjectLocus));
            LOG.info("  ( ( 2 * theta + ( 1 - theta ) * p{} ) / ( 4 * ( 1 + theta ) ) ) * ( 1 + ( ( 3 * ( 3 * theta + ( 1 - theta ) * p{} ) ) / ( 1 + 2 * theta ) ) )", subjectAllele.getAllele(), subjectAllele.getAllele());
            double p = _populationStatistics.getProbability(referenceLocus, subjectAllele);
            return ((2 * _theta + (1 - _theta) * p) / (4 * (1 + _theta))) * (1 + ((3 * (3 * _theta + (1 - _theta) * p)) / (1 + 2 * _theta)));
        } 
        // Parent and child alleles do not match
        LOG.info("Cousins Genotype Probability for reference {}, subject {} (aa-bb)", toLogString(referenceLocus), toLogString(subjectLocus));
        LOG.info("  ( 3 * ( 1 - theta ) * p{} * (theta + (1 - theta) * p{} ) ) / ( 4 * ( 1 + theta ) * ( 1 + 2 * theta ) )", subjectAllele.getAllele(), subjectAllele.getAllele());
        double p = _populationStatistics.getProbability(referenceLocus, subjectAllele);
        return (3 * (1 - _theta) * p * (_theta + (1 - _theta) * p)) / (4 * (1 + _theta) * (1 + 2 * _theta));
    }
    ...
    private double heterozygoteReferenceHomozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        if (referenceLocus.hasAllele(subjectAllele.getAllele())) {
            ...
            LOG.info("Cousins Genotype Probability for reference {}, subject {} (ab-aa)", toLogString(referenceLocus), toLogString(subjectLocus));
            LOG.info("  ( ( theta + ( 1 - theta ) * p{}) / ( 8 * ( 1 + theta ) ) ) * (  1 + ( ( 6 * (2 * theta + ( 1 - theta ) * p{} ) ) / ( 1 + 2 * theta ) ) )", subjectAllele, subjectAllele);
            return ((_theta + (1 - _theta) * pSubject) / (8 * (1 + _theta))) * (1 + ((6 * (2 * _theta + (1 - _theta) * pSubject)) / (1 + 2 * _theta)));
        }

        LOG.info("Cousins Genotype Probability for reference {}, subject {} (ab-cc)", toLogString(referenceLocus), toLogString(subjectLocus));
        LOG.info("  ( 3 * ( theta + ( 1 - theta ) * p{} ) * ( 1 - theta ) * p{} ) / ( 4 * ( 1 + theta ) * ( 1 + 2 * theta ) )", subjectAllele.getAllele(), subjectAllele.getAllele());
        return (3 * (_theta + (1 - _theta) * pSubject) * (1 - _theta) * pSubject) / (4 * (1 + _theta) * (1 + 2 * _theta));
    }
    ...
    private double heterozygoteReferenceHeterozygoteSubject(Locus referenceLocus, Locus subjectLocus) {
        ...
        switch (match.size()) {
            case 0: // No matching alleles (ab-cd)
                LOG.info("Cousins Genotype Probability for reference {}, subject {} (ab-cd)", toLogString(referenceLocus), toLogString(subjectLocus));
                Allele[] mismatchAlleles = mismatch.toArray(new Allele[2]);
                LOG.info("  ( 3 * ( 1 - theta ) * ( 1 - theta ) * p{} * p{} ) / ( 2 * ( 1 + theta ) * ( 1 + 2 * theta ) )", mismatchAlleles[0].getAllele(), mismatchAlleles[1]);
                double p = 3;
            ...
            case 1: // One matching allele (ab-ac)
                LOG.info("Cousins Genotype Probability for reference {}, subject {} (ab-ac)", toLogString(referenceLocus), toLogString(subjectLocus));
                ...
                LOG.info("  ( ( ( 1 - theta ) * p{} ) / ( 8 * (1 + theta))) * ( 1 + ( ( 12 * ( theta + (1 - theta) * p{} ) ) / ( 1 + 2 * theta ) ) ) ", mismatchAllele.getAllele(), matchAllele.getAllele());
                return ((_oneMinusTheta * pMismatch) / (8 * _onePlusTheta)) * (1 + ((12 * (_theta + (1 - _theta) * pMatch)) / (1 + 2 * _theta)));
            case 2: // All alleles match (ab-ab)
                LOG.info("Cousins Genotype Probability for reference {}, subject {} (ab-ab)", toLogString(referenceLocus), toLogString(subjectLocus));
                Allele[] alleles = match.toArray(new Allele[2]);
                LOG.info("  ( ( 1 / ( 8 * ( 1 + theta ) ) ) * ( 2 * theta + ( 1 - theta ) * ( p{} + p{} ) + ( 12 * ( theta + ( 1 - theta ) * p{} )* ( theta + ( 1 - theta ) * p{} ) ) / ( 1 + 2 * theta ) ) ) ", alleles[0], alleles[1], alleles[0], alleles[1]);
                ...
                return ((1 / (8 * (1 + _theta))) * (2 * _theta + (1 - _theta) * (pA + pB) + (12 * (_theta + (1 - _theta) * pA) * (_theta + (1 - _theta) * pB)) / (1 + 2 * _theta)));
            default:
